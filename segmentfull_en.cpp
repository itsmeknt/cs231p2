...
double getWeight(double px1r,double px1g,double px1b,double px2r,double px2g,double px2b,double beta,double gamma,int palpha,int nalpha)
{
    if (palpha==nalpha) return 0;
    else {
        double norm=(px1r-px2r)*(px1r-px2r)+(px1g-px2g)*(px1g-px2g)+(px1b-px2b)*(px1b-px2b);
        return gamma*exp(-beta*norm);
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	printf("1\n");
    //declare variables
    mxArray *D, *alphaIn, *beta, *gamma, *imData, *alpha;
    const mwSize *dimsData;
    double *DPtr, *alphaInPtr, *betaPtr, *gammaPtr, *imDataPtr, *alphaPtr;
    int dimxData, dimyData;
    int i,j;
    ...
    alphaIn = mxDuplicateArray(prhs[1]);
    beta = mxDuplicateArray(prhs[2]);
    gamma = mxDuplicateArray(prhs[3]);
    imData = mxDuplicateArray(prhs[4]);
    
    //figure out dimensions
	printf("3\n");
    dimsData = mxGetDimensions(prhs[4]);
    dimyData = (int)dimsData[0]; dimxData = (int)dimsData[1];
    

	printf("4\n");
    //associate outputs
    alpha = plhs[0] = mxCreateDoubleMatrix(1,dimxData*dimyData,mxREAL);
    
	
    //associate pointers
    alphaPtr = mxGetPr(alpha);
    DPtr = mxGetPr(D);
    alphaInPtr = mxGetPr(alphaIn);
    betaPtr = mxGetPr(beta);
    gammaPtr = mxGetPr(gamma);
    imDataPtr = mxGetPr(imData);
    
    double GammaVal=gammaPtr[0];
    double BetaVal=betaPtr[0];
    
	
    typedef Graph<double,double,double> GraphType;
	int numNodes=dimxData*dimyData;
	GraphType *g = new GraphType(/*estimated # of nodes*/ numNodes, /*estimated # of edges*/ numNodes*6); 
	g->add_node(numNodes);
    
    double pR,pG,pB,nR,nG,nB;
    double weight;
    int n1,n2,n3,n4,palpha,nalpha;
    // loop through pixels
    for (i=0;i<dimxData;i++)
    {
        
        for (j=0;j<dimyData;j++)
        {
            int xy = j+i*dimyData; // current pixel pointer index
            //printf("5\n");
            g->add_tweights(xy,DPtr[xy+numNodes],DPtr[xy]);
            pR=imDataPtr[xy];
            pG=imDataPtr[xy+numNodes];
            pB=imDataPtr[xy+2*numNodes];
            if (xy==1155) {
                printf(" i = %d, j = %d\n",i,j);
                printf("RGB = (%f,%f,%f)\n",pR,pG,pB);
                return;
            }
            palpha=(int)alphaInPtr[xy];
            printf("6: %d,%d,%d\n", i,j,xy);
            if (i<dimxData-1) {
                printf("61\n");
                n1=xy+dimyData;
                nR=imDataPtr[n1];
                nG=imDataPtr[n1+numNodes];
                nB=imDataPtr[n1+2*numNodes];
                nalpha=(int)alphaInPtr[n1];
                weight=getWeight(pR,pG,pB,nR,nG,nB,BetaVal,GammaVal,palpha,nalpha);
                g->add_edge(xy,n1,weight,weight);
            }
            ...
            if (j<dimyData-1) {
                ...
                n2=xy+1;
                nR=imDataPtr[n2];
                nG=imDataPtr[n2+numNodes];
                nB=imDataPtr[n2+2*numNodes];
                nalpha=(int)alphaInPtr[n2];
                weight=getWeight(pR,pG,pB,nR,nG,nB,BetaVal,GammaVal,palpha,nalpha);
                g->add_edge(xy,n2,weight,weight);
            }
            ...
            if (j>0) {
                ...
                n3=xy-1;
                nR=imDataPtr[n3];
                nG=imDataPtr[n3+numNodes];
                nB=imDataPtr[n3+2*numNodes];
                nalpha=(int)alphaInPtr[n3];
                weight=getWeight(pR,pG,pB,nR,nG,nB,BetaVal,GammaVal,palpha,nalpha);
                g->add_edge(xy,n3,weight,weight);
            }
            ...
            if (i>0) {
                ...
                n4=xy-dimyData;
                nR=imDataPtr[n4];
                nG=imDataPtr[n4+numNodes];
                nB=imDataPtr[n4+2*numNodes];
                nalpha=(int)alphaInPtr[n4];
                weight=getWeight(pR,pG,pB,nR,nG,nB,BetaVal,GammaVal,palpha,nalpha);
                g->add_edge(xy,n4,weight,weight);
            }
        }
    }
